    ...
    <section class="sidebar">
        ...
        <div class="user-panel">
            <div class="pull-left image">
                <img src="{{ asset('admin/img/user2-160x160.jpg') }}" class="img-circle" alt="{{ Auth::user()->name }}" onerror="this.src=location.origin+'/admin/img/avatar5.png'" />
            </div>
        ...
        @foreach (app()->sidebar->load() as $route => $menu)
            @isset($menu['can'])
            @can($menu['can'])
                @if($menu['type'] == 'header')
                ...
                <li class="@isset($menu['items']) treeview @else item @endif">
                    @if($menu['type'] == 'item')
                        @if(auth()->user()->has('publisher'))
                        <a href="{{ str_replace('{id}', auth()->user()->publisher->id, url($route)) }}"><i class="{{ $menu['icon'] }}"></i> <span>{{ $menu['text'] }}</span></a>
                        @else
